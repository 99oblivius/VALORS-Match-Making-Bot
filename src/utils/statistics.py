...
def create_stat_graph(graph_type: str, match_stats: List[MMBotUserMatchStats]) -> go.Figure:
    ...
    if graph_type == "mmr_time":
        ...
        title = "MMR over time"
        y_title = "MMR"
        fig = go.Figure(go.Scatter(x=x, y=y, mode='lines+markers', line=dict(color=VALORS_THEME1)))
    
    elif graph_type == "kills_game":
        y = [stat.kills for stat in match_stats]
        title = "Kills per game"
        y_title = "Kills"
        fig = go.Figure(go.Scatter(x=x, y=y, mode='markers', marker=dict(color=VALORS_THEME2)))
        ...
        y_title = "K/D Ratio"
        fig = go.Figure(go.Scatter(x=x, y=y, mode='lines+markers', line=dict(color=VALORS_THEME1_1)))
        ...
        y_title = "Win Rate"
        fig = go.Figure(go.Scatter(x=x, y=y, mode='lines', line=dict(color=VALORS_THEME1_2)))
    ...
    fig.update_layout(
        ...
        plot_bgcolor=VALORS_THEME1_2,
        paper_bgcolor=VALORS_THEME1_2)
    return fig